...
數據集、MANO 模型、SAM2/ProPainter 權重不在倉庫內。G1 120S 和 Galbot 仿真所需的 URDF/MJCF/mesh 已隨倉庫提供。
...
它們只用於「先移除真人手臂，再疊加 Galbot」的完整 egoview/LeRobot 流程。直接胸前相機疊加和普通 MuJoCo 回放不需要它們。

SAM2 必須能在 `simrender` 環境中 `import sam2`：
...
export PROPAINTER_PYTHON=/path/to/propainter/env/bin/python
```

`PROPAINTER_ROOT` 必須包含 `inference_propainter.py`。可選調節項包括 `PROPAINTER_CHUNK_SIZE`、`PROPAINTER_SUBVIDEO_LENGTH`、`PROPAINTER_MASK_DILATION` 和 `RETARGET_TMPDIR`。
...
源代碼沿用 Open-AoE 的 Apache-2.0 授權，見 [`LICENSE`](LICENSE)。Galbot G1 Golf 資產來自 `GalaxyGeneralRobotics/galbot_one_golf_description` 的 Apache-2.0 公開版本，並在資產目錄保留上游 LICENSE/README/commit。MANO、SAM2、ProPainter、LeRobot、AgiBot G1 120S 資產和數據集仍可能有各自授權或使用條款；商業使用或再分發前需核對原始來源條款。
